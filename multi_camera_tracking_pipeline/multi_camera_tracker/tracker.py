...
class Tracker:
    ...
    def process_frame(self, detections):
        ...
        if not predicted_positions or detection_positions.size == 0:
        ...
        for track_idx, detection_idx in zip(row_ind, col_ind):
            ...
            # The EKFTrack update method expects a 1D numpy array [x, y]
            measurement = detection_positions[detection_idx]
            self.tracks[track_idx].update(measurement, self.H, self.R)
        ...
        for detection_idx in range(len(detections)):
            if detection_idx not in matched_detection_indices:
                # The EKFTrack constructor expects a 1D numpy array [x, y]
                measurement = detection_positions[detection_idx]
                new_track = EKFTrack(measurement)
